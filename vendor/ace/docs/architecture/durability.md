...
1. Non-locking candidate query finds the lowest available pending/retrying
   sequence for a dispatchable run (terminal and `BLOCKED` runs excluded).
2. Outer transaction locks the `WorkflowRun`, then the `WorkflowInboxEvent`,
   and re-validates: the event must be the run's next pending sequence
   (**no overtaking**) and the run's `(workflow_name, workflow_version)`
   must be registered — otherwise the event is left `PENDING` untouched.
...
and never explicitly locked. Candidate queries are non-locking hints:
each service re-reads and validates rows after acquiring locks in order.
